...
async def test_task_group_cancels_siblings_on_failure():
    """When architect fails, downstream specialists must be cancelled."""
...
@pytest.mark.asyncio
async def test_downstream_agents_receive_architect_nodes():
    _test_node = {
        "id": "vpc",
        "type": "container",
        "position": {"x": 0, "y": 0},
    ...
    async def _coder(_requirements, _runtime, _start_time, **kwargs):
        coder_kwargs.update(kwargs)

    async def _description(_requirements, _runtime, _start_time, **kwargs):
    ...
    assert coder_kwargs.get("diagram_nodes") == [_test_node]
    assert description_kwargs.get("diagram_nodes") == [_test_node]
...
async def test_budget_cap_absent_keeps_normal_single_pass_behavior():
    ...
    assert specialist_calls == {"coder": 1, "description": 1}
...
async def test_budget_over_cap_retries_once_and_succeeds_when_under_cap():
    ...
    async def _coder(requirements, _runtime, _start_time, **_kwargs):
        specialist_calls["coder"] += 1
        coder_requirements.append(requirements)

    async def _description(_requirements, runtime, _start_time, **_kwargs):
        specialist_calls["description"] += 1
        if specialist_calls["description"] == 1:
    ...
    assert specialist_calls == {"coder": 2, "description": 2}
...
async def test_specialist_failure_does_not_fail_pipeline_and_reports_partial_summary():
    ...
    assert any(payload.get("type") == "done" for payload in runtime.sent_payloads)
    assert not any(payload.get("type") == "error" for payload in runtime.sent_payloads)
    assert _pipeline_event_exists(runtime, "description", "failed_after_retries")
...
async def test_specialist_retries_are_independent():
    ...
    assert specialist_calls == {"coder": 1, "description": 2}
    assert any(payload.get("type") == "done" for payload in runtime.sent_payloads)
    assert not any(payload.get("type") == "error" for payload in runtime.sent_payloads)
    assert _pipeline_event_exists(runtime, "description", "retrying")
    assert _pipeline_event_exists(runtime, "description", "completed")
