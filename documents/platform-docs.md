...
| `generation_agent_event` | `{ agent, status, event_type, message, history, started_at?, completed_at?, ts }` | Append-only live event stream per milestone; `history: true` appends to mini-history; `started_at`/`completed_at` are ISO 8601 timestamps |
| `generation_agent_update` | `{ mode: "initial_generation", agents: [{ agent, label, status, summary, detail, blocked_by, started_at, completed_at, elapsed_ms, progress_text, history, error }] }` | Structured per-agent observability snapshot for the generation panel; emitted after each `generation_agent_event`; also used for reconnect |
